...
class GeoTransformer(nn.Module):
    ...
    # Function to clamp transformation values
    def clamp_transform(self, transform, min_val, max_val):
        if transform.dim() == 2 and transform.shape == (4, 4):
            transform = transform.unsqueeze(0)  # Add a batch dimension

        assert transform.dim() == 3, "Transform tensor should have 3 dimensions after adjustment"

        transform[:, :3, 3] = torch.clamp(transform[:, :3, 3], min=min_val, max=max_val)
        return transform
    ...
    def forward(self, data_dict):
        ...
        with torch.no_grad():
            ...
            # Print the shape of estimated_transform before clamping
            print("Shape of estimated_transform before clamping:", estimated_transform.shape)
            
            #estimated_transform = self.clamp_transform(estimated_transform, min_val=-0.5, max_val=0.5)
