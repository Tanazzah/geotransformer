...
class ModelNetPairDataset(torch.utils.data.Dataset):
    ...
    def read_transform_matrix(self, index):
        with open("/home/tanazzah/GeoTransformer/experiments/dentskan/transform_matrices.txt", 'r') as f:
            lines = f.readlines()
            if index==0:
                start_line = 1 
            else:
                start_line = (index-1) * 5  # 4 lines of matrix + 1 gap line
            matrix_lines = lines[start_line:start_line + 4]
            
            matrix = []
            for line in matrix_lines:
                split_line = line.split()
                if len(split_line) == 4:
                    matrix.append(list(map(float, split_line)))
                else:
                    raise ValueError(f"Unexpected line format: {line}")
            
            matrix = np.array(matrix)
            if matrix.shape != (4, 4):
                raise ValueError(f"Matrix shape is incorrect: {matrix.shape}")
            print(matrix)
            f.close()
            return matrix
    ...
    def _convert_ply_to_dict(self,index):
        ...
        pcd1 = o3d.io.read_point_cloud(ply_file_path_1)
    ...
    def __getitem__(self, index):
        ...
        #data_dict: Dict = self.data_files[index]
        data_dict: Dict
        
        data_dict=self._convert_ply_to_dict(index)
        #print(index)
        ...
        if index!=0:
            transform_matrix = self.read_transform_matrix(index)
            rotation = transform_matrix[:3, :3]
            translation = transform_matrix[:3, 3]
            #ref_points = np.matmul(ref_points, rotation.T) + translation
            #ref_normals = np.matmul(ref_normals, rotation.T)
        #   ref_points, ref_normals = apply_transform(ref_points, transform_matrix, normals=ref_normals)
        ...
        '''rootsave='/home/tanazzah/GeoTransformer/data/dentskan/ply2'
        ref_ply_path = osp.join(rootsave, f'{index}.ply')
