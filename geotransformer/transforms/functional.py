...
def convert_ply_to_dict(index):
    data_list = []
    folder_path="ply2"
    ply0=str(index)+".ply"
    ply1=str(index)+".ply"
    ply_file_path = os.path.join(folder_path, ply0)
    points0 = np.asarray(o3d.io.read_point_cloud(ply_file_path).points)
    normals0 = compute_normals(points0)
    points1 = np.asarray(o3d.io.read_point_cloud(os.path.join(folder_path, ply1)).points)
...
def random_jitter_points(points, scale, noise_magnitude):
